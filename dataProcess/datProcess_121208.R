...
olddat <- read.table("dataProcess/scHemeData_v2.txt", header = TRUE, row.names = NULL, stringsAsFactors = FALSE)[,c(1,2)]
olddat[,2] <- 1:nrow(olddat)
dd <- merge(olddat, dat, by.x = "Name", by.y = "Name", sort = FALSE)
dat <- dd[order(dd[,2]),]
cellnames <- dat[,1]
